...
- Added migration 009's permanent authorization/completion ledger and the
  fifth recovery lane, including cold-start paging, without granting mutation
  authority to the stopped-only reconciler.
...
- Only owner launch/stop finalizers that commit exact `complete-stopped`
  ...
  009 binds that authorization to both the terminal operation and launch
  attempt, retains the exact stopped revision 4 `terminalRecord` and digests,
  and permanently records `collected` or `absent` completion. The stopped-only
  launch reconciler remains a pure read.
- In the assembled production runner, the fifth `supervisor-state-gc` recovery
  ...
  lifecycle lease. It pages pending authorizations, invokes the independently
  settled `supervisorStateCollector.collectTerminalState` physical leaf with
  the exact terminal record, and commits the collection receipt. Cold-start
  recovery and exact ledger readback preserve progress across process restart.
...
- The transient raw Podman supervisor is version 3, its launch receipt is
  version 2, and the logical facade is version 2. The durable logical launch
  request, evidence projection, and local revision records remain version 1,
  so the response-ABI upgrade does not change existing request hashes.
